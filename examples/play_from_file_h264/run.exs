...
defmodule Stopwatch do
  ...
  def wait_for_peer_connection_close(pc) do
    receive do
...
:ok =
  Specter.TrackLocalStaticSample.play_from_file_h264(
    specter,
    track,
    "examples/play_from_file_h264/sample_video.h264"
  )

:ok = Stopwatch.wait_for_playback_finished(track)

:ok = Specter.PeerConnection.close(specter, pc)
:ok = Stopwatch.wait_for_peer_connection_close(pc)
